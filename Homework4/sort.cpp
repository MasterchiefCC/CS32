...
void insertion_sort(vector<Student>& s, bool comp(const Student&, const Student&))
{
	for (int a = 0; a < s.size(); a++) {
		Student temp = s[a];
		int b = a;
		while (b > 0 && comp(temp, s[b - 1])) { s[b] = s[b - 1]; b--; }
		s[b] = temp;
	}
...
int main()
{
	...
	// destruction time.
	{
		...
		vector<Student*> tempstd;
		for (int a = 0; a < auxStudents.size(); a++)tempstd.push_back(&auxStudents[a]);
		sort(tempstd.begin(), tempstd.end(), compareStudentPtr);
		for (int a = 0; a < students.size(); a++) students[a] = *tempstd[a];
